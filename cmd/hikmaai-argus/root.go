// ABOUTME: Root command for hikmaai-argus CLI
...
func newRootCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "hikmaai-argus",
		Short: "HikmaArgus - Stateless signature-based antivirus service",
		Long: `HikmaArgus is a stateless, signature-based antivirus service that provides
...
feed management for ClamAV and abuse.ch signatures.`,
	...
	// Global flags.
	cmd.PersistentFlags().StringVarP(&cfgFile, "config", "c", "", "config file (default: $HOME/.hikmaai-argus/config.yaml)")
...
func newVersionCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("hikmaai-argus version %s\n", version)
